...
const loadUserProfiles = async () => {
  ...
  for (const user of users) {
    ...
    await Promise.all([redisClient.hset(`user:${user.username}`, user), redisClient.sadd('usernames', user.username)]);
  }
};
...
const verifySearch = async () => {
  const userSearchResults = await redisClient.call(
    'FT.SEARCH',
    'idx:users',
    '@firstname:Simon @city:{Nottingham}',
    'LIMIT',
    '0',
    '999999'
  );
  const messageSearchResults = await redisClient.call(
    'FT.SEARCH',
    'idx:messages',
    '@channel:{cricket|tech} @username:{wasim}',
    'LIMIT',
    '0',
    '999999'
  );
